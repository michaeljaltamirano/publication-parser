...
import { fetchContent, getOptions, throwCookieError, handleError, isNotNullish, writeHtmlFile, getEpub, clearNewlinesAndFormatting, } from '../utils.js';
const { JSDOM } = jsdom;
...
const getFeatureArticleContent = (articleLayoutFeature) => {
    ...
    const featureLayoutHeader = newArticleLayoutFeature.querySelector('.title-header.desktop')?.outerHTML ??
        '';
...
const getSimpleArticleContent = (articleLayoutSimple) => {
    ...
    headerMeta?.remove();
    const title = simpleLayoutHeader?.querySelector('h1.title');
...
export default async function harpersParser(issueUrl) {
    const headers = {
        cookie,
    };
    ...
    return processHrefs(newRefs, clearNewlinesAndFormatting(volumeNumberAndDate), options);
}
