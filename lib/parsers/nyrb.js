...
const processContent = (articleDom, dom) => {
    ...
    const titleAndAuthorMarkup = `<h2 class="chapter">${title?.innerHTML ?? ''}</h2><h3>${author?.textContent ?? ''}</h3><h3>${dek?.outerHTML ?? ''}</h3>`;
    ...
    dom.window.document.body.innerHTML = `${dom.window.document.body.innerHTML}<div class="article-container">${titleAndAuthorMarkup}${reviewedItems?.outerHTML ?? ''}${bodyMarkup}${authorInfo?.outerHTML ?? ''}</div>`;
    return undefined;
};
...
export default async function nyrbParser(issueUrl) {
    const headers = {
        cookie,
    };
    ...
    const date = dom.window.document.querySelector('header.issue_header > p.h2')?.textContent;
