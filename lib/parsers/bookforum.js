...
import { fetchContent, fetchContentArrayBuffer, getOptions, throwCookieError, handleError, isNotNullish, getEpub, writeHtmlFile, } from '../utils.js';
const { JSDOM } = jsdom;
...
const getArticleString = (article) => {
    ...
    const chapter = article.querySelector('h1.blog-article__header')
...
export default async function bookforumParser(issueUrl) {
    const headers = {
        cookie,
    };
